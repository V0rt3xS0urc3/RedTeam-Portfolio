<div align="center">

# 🔐 Red Team Portfolio

**Diego Arriagada Zamora | V0rt3x_S0urc3**

<img src="/img/Micaricatura.png" width="600">

### 🎯 Pentester | Red Team | Hacker Ético

[![LinkedIn](https://img.shields.io/badge/LinkedIn-diego_arriagada_zamora-0077B5?style=for-the-badge&logo=linkedin&logoColor=white&labelColor=101010)](https://www.linkedin.com/in/diegoarriagadazamora)
[![Instagram](https://img.shields.io/badge/Instagram-@diego_arriagadadev-E4405F?style=for-the-badge&logo=instagram&logoColor=white&labelColor=101010)](https://instagram.com/diego_arriagadadev)
[![GitHub](https://img.shields.io/badge/GitHub-V0rt3xS0urc3-181717?style=for-the-badge&logo=github&logoColor=white&labelColor=101010)](https://github.com/V0rt3xS0urc3)

</div>

---

## 📂 Proyectos y Recursos

<div align="center">

### 🏠 [Red Doméstica Segmentada](./RedDomestica)

**Infraestructura de red empresarial aplicada a un entorno doméstico**

<img src="https://img.shields.io/badge/Cisco-892FSP-1BA0D7?style=for-the-badge&logo=cisco&logoColor=white"> <img src="https://img.shields.io/badge/OpenWrt-EDUP-00B5E2?style=for-the-badge&logo=openwrt&logoColor=white"> <img src="https://img.shields.io/badge/VLANs-6_Segmentadas-0052CC?style=for-the-badge"> <img src="https://img.shields.io/badge/QoS-Gaming_Priority-000?style=for-the-badge&logo=data:image/svg%2Bxml;base64,) <img src="https://img.shields.io/badge/VPN-WireGuard-0171CE?style=for-the-badge&logo=wireguard&logoColor=white">

*Diseño, configuración e implementación de una red doméstica segmentada usando un* **Cisco 892FSP** *como núcleo, con* **6 VLANs** *para aislar tráfico de cámaras, gaming, multimedia, smartphones, invitados y administración. Incluye* **QoS** *para priorizar gaming,* **WireGuard** *para acceso remoto seguro, y puntos de acceso* **WiFi 6** *con OpenWrt.*

[**📖 Ver Documentación Completa**](./RedDomestica)

---

### 🐳 [Kali Portable Full - Red Team Edition](./kali-portable)

**Estación de pentesting completa en Docker con aceleración GPU**

<img src="https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white"> <img src="https://img.shields.io/badge/Kali_Linux-557C94?style=for-the-badge&logo=kali-linux&logoColor=white"> <img src="https://img.shields.io/badge/NVIDIA_GPU-76B900?style=for-the-badge&logo=nvidia&logoColor=white">

</div>

✅ 100+ herramientas de pentesting instaladas  
✅ Aceleración GPU para Hashcat  
✅ Scripts automatizados para WPA2, modo monitor, y más  
✅ Volumen persistente para tus datos  
✅ Compatible con TryHackMe, HackTheBox, y HackLabs  

[**📖 Ver Documentación Completa**](./kali-portable)

**Desarrollado por:**

- 🧑‍💻 **V0rt3x_S0urc3** - [GitHub](https://github.com/V0rt3xS0urc3)
- 🤖 **Qwen Studio IA** - [Chat](https://chat.qwen.ai)

*Proyecto desarrollado con asistencia de inteligencia artificial*

---

### 🎯 [WriteUps DockerLabs](./WriteupsDockerLabs)

**Soluciones detalladas de máquinas vulnerables de DockerLabs.es**

<img src="https://img.shields.io/badge/DockerLabs-193_Máquinas-blue?style=for-the-badge"> <img src="https://img.shields.io/badge/Niveles-Muy_Fácil_a_Difícil-green?style=for-the-badge">

📚 Categorías:
- 🔧 Hacking Infraestructura
- 🌐 Hacking Web
- 📝 Hacking CMS
- 🐛 Bug Bounty
- 🔀 Pivoting

[**📖 Explorar WriteUps**](./WriteupsDockerLabs)

---

### 🏴‍☠️ WriteUps HackLabs

**Próximamente: Soluciones de máquinas de HackLabs**

<img src="https://img.shields.io/badge/Estado-En_Desarrollo-orange?style=for-the-badge">

🚧 Esta sección está en construcción. Pronto agregaré writeups detallados de máquinas de HackLabs.

[**🔗 Visitar HackLabs en GitHub**](https://github.com/afsh4ck/HackLabs)


---

## 🚀 Inicio Rápido: Kali Portable

<div align="center">

### Instalación en 3 comandos:

```bash
# 1. Clonar el repositorio
git clone https://github.com/V0rt3xS0urc3/RedTeam-Portfolio.git
cd RedTeam-Portfolio/kali-portable

# 2. Ejecutar el instalador automático
chmod +x scripts/install-on-new-pc.sh
./scripts/install-on-new-pc.sh

# 3. Iniciar el entorno
./run-kali.sh normal

Tiempo de instalación: ~30-45 minutos
```
</div>

👤 Sobre Mí


Diego Arriagada Zamora (V0rt3xS0urc3)

Pentester y Hacker Ético chileno especializandome en:

    🔴 Red Team Operations
    🌐 Auditoría Web y de Infraestructura
    📡 Auditoría WiFi y Cracking
    🏢 Active Directory Penetration Testing
    🔬 Análisis Forense Digital


⚠️ Disclaimer
<div align="center">

Este repositorio es SOLO para fines educativos y auditorías autorizadas.
El uso indebido de estas herramientas es responsabilidad exclusiva del usuario. 
Respeta siempre las leyes y obtén autorización antes de realizar pruebas de penetración.
**🇨🇱 Chile - Ley 19.223:** El acceso no autorizado a sistemas informáticos es un delito.

</div>


  
🔥 ¿Te gustó el proyecto?
¡Dale una estrella ⭐ en GitHub!

### 🎯 Pentester | Red Team | Hacker Ético

[![LinkedIn](https://img.shields.io/badge/LinkedIn-diego_arriagada_zamora-0077B5?style=for-the-badge&logo=linkedin&logoColor=white&labelColor=101010)](https://www.linkedin.com/in/diegoarriagadazamora)
[![Instagram](https://img.shields.io/badge/Instagram-@diego_arriagadadev-E4405F?style=for-the-badge&logo=instagram&logoColor=white&labelColor=101010)](https://instagram.com/diego_arriagadadev)
[![GitHub](https://img.shields.io/badge/GitHub-V0rt3xS0urc3-181717?style=for-the-badge&logo=github&logoColor=white&labelColor=101010)](https://github.com/V0rt3xS0urc3)
